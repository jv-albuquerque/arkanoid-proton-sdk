...
uint8_t ArduboyCore::buttonsState()
{
  uint8_t buttons = 0;
	  ...
	  if (g_gameKeys.buttonStatus[GAME_KEY_LEFT])
	  {
		  buttons |= LEFT_BUTTON;
	  }
	  if (g_gameKeys.buttonStatus[GAME_KEY_RIGHT])
	  {
		  buttons |= RIGHT_BUTTON;
	  }
	  if (g_gameKeys.buttonStatus[GAME_KEY_DOWN])
	  {
		  buttons |= DOWN_BUTTON;
	  }
	  if (g_gameKeys.buttonStatus[GAME_KEY_UP])
	  {
		  buttons |= UP_BUTTON;
	  }

	  if (g_gameKeys.buttonStatus[GAME_KEY_A])
	  {
		buttons |= A_BUTTON;
	  }
	  if (g_gameKeys.buttonStatus[GAME_KEY_B])
	 {
		  buttons |= B_BUTTON;
	  }
